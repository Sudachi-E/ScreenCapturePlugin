...
bool writeAVI(const std::string &path, RingBuffer &buf,
    ...
    buf.lock();
    uint32_t srcFrameCount = buf.count();

    if (srcFrameCount == 0) {
        buf.unlock();
        DEBUG_FUNCTION_LINE("writeAVI: no frames to write\n");
        return false;
    }

    constexpr uint32_t framePeriodUs = 1000000u / 30;

    uint32_t neededOut = srcFrameCount * 2;
    if (neededOut > sOutCap) {
        uint32_t *newBuf = (uint32_t *) realloc(sOutBuf, neededOut * sizeof(uint32_t));
        if (!newBuf) {
            buf.unlock();
            DEBUG_FUNCTION_LINE("writeAVI: out of memory for outFrames\n");
            return false;
        }
        sOutBuf = newBuf;
        sOutCap = neededOut;
    ...
    for (uint32_t i = 0; i < srcFrameCount; i++) {
        ...
        if (i + 1 < srcFrameCount) {
            uint64_t dt   = buf.get(i + 1)->timestamp - buf.get(i)->timestamp;
            ...
            if (dtUs > framePeriodUs) {
                ...
                for (uint32_t d = 0; d < extraFrames; d++) {
                    if (outCount >= sOutCap) {
                        ...
                        if (!newBuf) {
                            buf.unlock();
                            DEBUG_FUNCTION_LINE("writeAVI: out of memory for outFrames (dup)\n");
                            return false;
                        }
                        sOutBuf = newBuf;
                        sOutCap = newCap;
    ...
    for (uint32_t i = 0; i < srcFrameCount; i++) {
        if (buf.get(i)->size > maxFrameSize)
            maxFrameSize = (uint32_t) buf.get(i)->size;
    }

    uint32_t videoWidth  = buf.get(0)->width;
    uint32_t videoHeight = buf.get(0)->height;
    ...
    DEBUG_FUNCTION_LINE("writeAVI: %u src frames -> %u output frames at 30 fps%s\n",
                        srcFrameCount, totalFrames,
                        hasAudio ? " + PCM audio" : "");

    FILE *fp = fopen(path.c_str(), "wb");
    if (!fp) {
        buf.unlock();
        DEBUG_FUNCTION_LINE("writeAVI: failed to open %s\n", path.c_str());
        return false;
    }
...

    {
        ...
        writeU32LE(fp, 0);
        writeU32LE(fp, 0x10);
        writeU32LE(fp, totalFrames);
        writeU32LE(fp, 0);
        writeU32LE(fp, hasAudio ? 2u : 1u);
        writeU32LE(fp, maxFrameSize);
        writeU32LE(fp, videoWidth);
        writeU32LE(fp, videoHeight);
...

    {
...

        {
            ...
            writeU32LE(fp, totalFrames);
            writeU32LE(fp, maxFrameSize);
            writeU32LE(fp, (uint32_t)-1);
            writeU32LE(fp, 0);
            writeU16LE(fp, 0);
            writeU16LE(fp, 0);
            writeU16LE(fp, (uint16_t)videoWidth);
            writeU16LE(fp, (uint16_t)videoHeight);
            endChunk(fp, sz);
        }

        {
            ...
            writeU32LE(fp, 40);
            writeI32LE(fp, (int32_t)videoWidth);
            writeI32LE(fp, (int32_t)videoHeight);
            writeU16LE(fp, 1);
            writeU16LE(fp, 24);
            fwrite("MJPG", 1, 4, fp);
            writeU32LE(fp, maxFrameSize);
    ...
    if (hasAudio) {
...

        {
            ...
            writeU32LE(fp, 0);
            writeU32LE(fp, 1);
            writeU32LE(fp, audioSampleRate);
            writeU32LE(fp, 0);
            writeU32LE(fp, audioStreamLen);
            writeU32LE(fp, audioByteSize);
            writeU32LE(fp, 0);
            writeU32LE(fp, audioBlockAlign);
            writeU16LE(fp, 0);
            writeU16LE(fp, 0);
            writeU16LE(fp, 0);
            writeU16LE(fp, 0);
    ...
    if (numIdxEntries > sIdxCap) {
        ...
        if (!newBuf) {
            buf.unlock();
            fclose(fp);
            DEBUG_FUNCTION_LINE("writeAVI: out of memory for index\n");
            return false;
        }
        sIdxBuf = newBuf;
    ...
    for (uint32_t i = 0; i < totalFrames; i++) {
        const CapturedFrame *frame = buf.get(sOutBuf[i]);
    ...
    // Audio chunk (after all video frames)
    if (hasAudio) {
        uint32_t chunkOffset = (uint32_t)(ftell(fp) - moviDataStart);
        writeFourCC(fp, "01wb");
        writeU32LE(fp, audioByteSize);
    ...
    DEBUG_FUNCTION_LINE("writeAVI: done -> %s\n", path.c_str());
    return true;
}
